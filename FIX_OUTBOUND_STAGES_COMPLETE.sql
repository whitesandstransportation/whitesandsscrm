...
-- Check if all deal stages are now covered
SELECT 
  d.stage,
  COUNT(*) as deal_count,
  CASE 
    WHEN d.stage = ANY(
      SELECT jsonb_array_elements_text(p.stages)
      FROM pipelines p
      WHERE p.name = 'Outbound Funnel'
    ) THEN '✓ Covered'
    ELSE '✗ MISSING'
  END as status
FROM deals d
WHERE d.pipeline_id = (SELECT id FROM pipelines WHERE name = 'Outbound Funnel')
GROUP BY d.stage
ORDER BY deal_count DESC;
